...
async def startup_event():
    ...
    # Auto-create admin account from env var on first boot
    _admin_pwd = os.getenv("ODYSSEUS_ADMIN_PASSWORD", "").strip()
    if _admin_pwd and not auth_manager.is_configured:
        _ok = auth_manager.setup("admin", _admin_pwd)
        if _ok:
            logger.info("Auto-created admin account from ODYSSEUS_ADMIN_PASSWORD")
        else:
            logger.warning("Failed to auto-create admin account")
    # Auto-enable open registration so users can sign up with their apinet.cloud key
